...
const EditModeToggle = () => {
  ...
  const handleCopyForAI = () => {
    ...
    if (!hasChanges) {
      alert("Вы еще не внесли никаких изменений.");
      return;
    }

    const messageForAI = `Привет! Я изменил тексты на сайте. Вот новые значения (JSON):\n\n\`\`\`json\n${JSON.stringify(data, null, 2)}\n\`\`\`\n\nПожалуйста, примени эти изменения к коду.`;
    
    navigator.clipboard.writeText(messageForAI).then(() => {
      setCopied(true);
  ...
  return (
    <div className="fixed bottom-24 right-8 flex flex-col gap-3 z-50">
      {isEditMode && (
        ...
        >
          {copied ? <CheckCircle2 className="w-5 h-5 text-emerald-500" /> : <Copy className="w-5 h-5 text-primary" />}
          {copied ? "Скопировано!" : "Скопировать правки для ИИ"}
        </button>
      )}
      <button 
        ...
        <div className="flex items-center gap-8">
          ...
          <div className="hidden lg:flex items-center gap-6">
            <a href="#" onClick={e => isEditMode && e.preventDefault()} className="text-sm font-bold text-[#111827] hover:text-primary transition-colors">
              <EditableText id="nav-link-1" defaultText="" />
            </a>
          </div>
        </div>
    ...
    <div className="grid md:grid-cols-2 gap-8">
      <div className="alfa-card bg-white border-slate-100 shadow-sm p-10 flex flex-col justify-between">
        <div>
          ...
          <EditableText id="jtbd-1-desc" as="p" className="text-lg text-slate-600 leading-relaxed mb-8" defaultText="Помогаем быстро проверить подержанный автомобиль и отсеять проблемные варианты. Сохраните свои деньги на диагностике в СТО и получите уверенность в том, что выбираете надежную машину." />
        </div>
        <div className="flex items-center gap-2 text-primary font-bold">
          <EditableText id="jtbd-1-badge" defaultText="Безопасность" /> <CheckCircle2 className="w-5 h-5" />
        </div>
      </div>
      <div className="alfa-card bg-white border-slate-100 shadow-sm p-10 flex flex-col justify-between">
        <div>
          ...
          <EditableText id="jtbd-2-desc" as="p" className="text-lg text-slate-600 leading-relaxed mb-8" defaultText="Оформите понятный электронный отчет о реальном состоянии вашего автомобиля. Наличие независимой проверки в объявлении повышает лояльность покупателей и значительно ускоряет процесс продажи." />
        </div>
        <div className="flex items-center gap-2 text-primary font-bold">
          <EditableText id="jtbd-2-badge" defaultText="Ликвидность" /> <CheckCircle2 className="w-5 h-5" />
        </div>
      </div>
    </div>
  </section>
  ...
  <section id="pricing" className="py-24 bg-white">
    <div className="max-w-7xl mx-auto px-4">
      <div className="text-center mb-16">
        ...
        <EditableText id="price-main-desc" as="p" className="section-subtitle mx-auto" defaultText="\n" />
      </div>
      ...
      <div className="grid md:grid-cols-4 gap-12 mb-16">
        ...
        <div>
          ...
          <ul className="space-y-4 text-slate-400">
            ...
            <li className="flex items-center gap-2"><MessageSquare className="w-4 h-4" /> <EditableText id="footer-contact-2" defaultText="WhatsApp: +996 (777) 800-300" /></li>
          </ul>
        </div>
      </div>
